...
function StartX.align_center(tbl, state)
...
function StartX.pad_margin(tbl, state, margin, shrink)
    local longest = longest_line(tbl)
    local left
    if shrink then
...
function StartX.highlight(state, end_ln, hl, left)
...
function StartX.resolve(to, el, opts, state)
...
function layout_element.text(el, conf, state)
    if type(el.val) == "table" then
        ...
        if conf.opts and conf.opts.margin and el.opts and (el.opts.position ~= "center") then
            local left
            val, left = StartX.pad_margin(val, state, conf.opts.margin, if_nil(el.opts.shrink_margin, true))
            padding.left = padding.left + left
        end
        if el.opts then
            if el.opts.position == "center" then
                local left
                val, left = StartX.align_center(val, state)
                padding.left = padding.left + left
            end
        ...
        if el.opts and el.opts.hl then
            hl = StartX.highlight(state, end_ln, el.opts.hl, padding.left)
        end
        state.line = end_ln
        return val, hl
    end
    ...
    if type(el.val) == "string" then
        ...
        if conf.opts and conf.opts.margin and (not el.opts or el.opts.position ~= "center") then
            local left
            val, left = StartX.pad_margin(val, state, conf.opts.margin, if_nil(el.opts and el.opts.shrink_margin, true))
            padding.left = padding.left + left
        end
        if el.opts then
            if el.opts.position == "center" then
        ...
        if el.opts and el.opts.hl then
            hl = StartX.highlight(state, state.line, el.opts.hl, padding.left)
        end
    ...
    if type(el.val) == "function" then
        return StartX.resolve(layout_element.text, el, conf, state)
    end
end
...
function layout_element.button(el, conf, state)
    ...
    if el.opts and el.opts.hl_shortcut then
        ...
        if el.opts.align_shortcut == "right" then
            hl = StartX.highlight(state, state.line, hl, #el.val + padding.center)
        else
            hl = StartX.highlight(state, state.line, hl, padding.left)
        end
    end
    ...
    if el.opts and el.opts.hl then
        ...
        list_extend(hl, StartX.highlight(state, state.line, el.opts.hl, left))
    end
    state.line = state.line + 1
    return val, hl
end
...
function layout_element.group(el, conf, state)
    if type(el.val) == "function" then
        return StartX.resolve(layout_element.group, el, conf, state)
    end
...
function keymaps_element.group(el, conf, state)
    if type(el.val) == "function" then
        StartX.resolve(keymaps_element.group, el, conf, state)
    end
...
-- stylua: ignore start
local function enable_StartX(conf, state)
    ...
    local group_id = vim.api.nvim_create_augroup('StartX_temp', { clear = true })

    vim.api.nvim_create_autocmd('BufUnload', {
        group = group_id,
        pattern = '<buffer>',
        callback = StartX.close,
    })

    vim.api.nvim_create_autocmd('CursorMoved', {
        group = group_id,
        pattern = '<buffer>',
        callback = function() StartX.move_cursor(state.window) end,
    })
    ...
    if conf.opts then
        if if_nil(conf.opts.redraw_on_resize, true) then
            vim.api.nvim_create_autocmd('VimResized', {
                group = group_id,
                pattern = '*',
                callback = function() StartX.redraw(conf, state) end,
            })
...
-- stylua: ignore start
local function should_skip_StartX()
...
function StartX.draw(conf, state)
    cursor_jumps = {}
    cursor_jumps_press = {}
...
function StartX.move_cursor(window)
...
function StartX.redraw(conf, state)
    conf = conf or current_config
    state = state or current_state
    if state.open then
        StartX.draw(conf, state)
    end
end

function StartX.close(_, state)
    state = state or current_state
    state.open = false
    cursor_ix = 1
    cursor_jumps = {}
    vim.api.nvim_del_augroup_by_name("StartX_temp")
    vim.api.nvim_exec_autocmds("User", { pattern = "StartXClosed" })
end

function StartX.start(on_vimenter, conf)
    local window = vim.api.nvim_get_current_win()

    local buffer
    if on_vimenter then
        if should_skip_StartX() then
            return
        end
        buffer = vim.api.nvim_get_current_buf()
    else
        if vim.bo.ft ~= "StartX" then
    ...
    vim.keymap.set("n", "<CR>", function() StartX.press() end, { noremap = false, silent = true, buffer = state.buffer })
    vim.keymap.set("n", "<M-CR>", function() StartX.queue_press() end, { noremap = false, silent = true, buffer = state.buffer })

    current_state = state

    enable_StartX(conf, state)

    StartX.draw(conf, state)

    vim.api.nvim_exec_autocmds("User", { pattern = "StartXReady" })
    keymaps(conf, state)
end

function StartX.setup()
    local config = require"plugins.StartX.resource.themes.startify".config
    vim.validate({
        config = { config, "table" },
    ...
    vim.api.nvim_create_user_command("StartX", function(_)
        StartX.start(false)
    end, {
        bang = true,
        desc = 'require"StartX".start(false)',
    })

    vim.api.nvim_create_user_command("StartXRedraw", function(_) StartX.redraw() end, {
        bang = true,
        desc = 'require"StartX".redraw()',
    })

    local group_id = vim.api.nvim_create_augroup("StartX_start", { clear = true })
...
StartX.layout_element = layout_element
StartX.keymaps_element = keymaps_element
